...
import type { AnimalHomeOptions } from '../Models/Animal.model';

const ANIMAL_HOME_MODEL_PATH = 'assets/gltf/animal_home.glb';
...
const SPAWN_ANIMATION_DURATION_SECONDS = 0.52;
const SPAWN_START_SCALE_MULTIPLIER = 0.08;
const SPAWN_RISE_HEIGHT = 0.12;
const SPAWN_SQUASH_AMOUNT = 0.12;

export class AnimalHome extends THREE.Group {
    ...
    constructor(maxTextureAnisotropy: number, options: AnimalHomeOptions = {}) {
        super();
        this.name = 'AnimalHome';
    ...
    load() {
        ...
        this.loadPromise = new Promise((resolve, reject) => {
            this.loader.load(
                ...
                (error) => {
                    this.isLoading = false;
                    console.error(`Failed to load animal home model: ${ANIMAL_HOME_MODEL_PATH}`, error);
                    reject(error);
                },
            );
        });
    ...
    update(deltaSeconds: number) {
        ...
        this.spawnElapsedSeconds += Math.max(0, deltaSeconds);
    ...
    private applySpawnAnimationFrame(elapsedSeconds: number) {
        ...
        const xzScale = scaleMultiplier * (1 + squashWave * 0.4);
